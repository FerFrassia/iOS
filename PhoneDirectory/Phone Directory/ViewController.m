...
- (IBAction)save:(id)sender {
    ...
    if(sqlite3_open(dbpath, &_DB) == SQLITE_OK) {
        ...
        if(sqlite3_step(statement) == SQLITE_DONE) {
            [self showUIAlertWithMessage:@"User added to the database" andTitle:@"Hurray!"];
            _name.text = @"";
            _address.text = @"";
            _phone.text = @"";
        }
    ...
    if(sqlite3_open(dbpath, &_DB) == SQLITE_OK) {
        ...
        if(sqlite3_prepare_v2(_DB, query_statement, -1, &statement, NULL) == SQLITE_OK) {
            if(sqlite3_step(statement) == SQLITE_ROW) {
                ...
                [self showUIAlertWithMessage:@"Match found" andTitle:@"Hurray!"];
            }
            else {
                [self showUIAlertWithMessage:@"Match not found" andTitle:@"Error"];
                _address.text = @"";
                _phone.text = @"";
            }
        }
...
- (IBAction)remove:(id)sender {
    ...
    if(sqlite3_open(dbpath, &_DB) == SQLITE_OK){
        ...
        if(sqlite3_exec(_DB, query_statement, NULL, NULL, &errorMessage) == SQLITE_OK){
            [self showUIAlertWithMessage:@"Deleted from database" andTitle:@"Hurray!"];
        }
        else{
